...
describe.skipIf(shouldSkip)("Database Query Security Audit", () => {
  ...
  describe("Secrets Table - User Isolation", () => {
    ...
    it("prevents User A from updating User B secret", async () => {
      ...
      const result = await secretsService.update(secretBId, userAId, {
        title: "Hacked Title",
      })
    ...
    it("allows User A to update their own secret", async () => {
      ...
      const result = await secretsService.update(secretAId, userAId, {
        title: newTitle,
      })
  ...
  describe("Check-in Tokens - Ownership Validation", () => {
    ...
    afterEach(async () => {
      try {
        await db
          .delete(checkInTokens)
          .where(eq(checkInTokens.secretId, secretAId))
        await db
          .delete(checkInTokens)
          .where(eq(checkInTokens.secretId, secretBId))
      } catch (error) {
    ...
    it("prevents User A from accessing User B check-in tokens directly", async () => {
      ...
      const result = await db
        ...
        .where(and(eq(checkInTokens.id, tokenBId), eq(secrets.userId, userAId)))
    ...
    it("allows User A to access their own check-in tokens", async () => {
      const result = await db
        ...
        .from(checkInTokens)
        .innerJoin(secrets, eq(checkInTokens.secretId, secrets.id))
        .where(and(eq(checkInTokens.id, tokenAId), eq(secrets.userId, userAId)))
  ...
  describe("API Route Integration", () => {
    ...
    it("verifies update operations include userId filtering", async () => {
      ...
      const result = await db
        ...
        .set({ title: "Hacked" })
        .where(
          and(eq(secrets.id, secretBId), eq(secrets.userId, sessionUserId)),
        )
        .returning()
