...
describe('Enhanced Email Verification Service', () => {
  ...
  describe('createVerificationToken', () => {
    ...
    it('should create tokens with 24-hour expiration', async () => {
      ...
      expect(actualExpiration).toBeLessThanOrEqual(endTime + 24 * 60 * 60 * 1000);
    });
    ...
    it('should generate cryptographically secure tokens', async () => {
      ...
      results.forEach((result, index) => {
        console.log(`Token generation ${index}:`, result.success ? `SUCCESS (${result.token?.substring(0, 8)}...)` : `FAILED: ${result.error}`);
      });
  ...
  describe('checkEmailVerification', () => {
    it('should return verification status for existing user', async () => {
      ...
      const { checkEmailVerification } = await import('@/lib/auth/email-verification');
  ...
  describe('Enhanced Security Features', () => {
    it('should implement token cleanup for expired tokens', async () => {
      ...
      const { cleanupExpiredTokens } = await import('@/lib/auth/email-verification');
    ...
    it('should validate token format and structure', async () => {
      ...
      const { validateTokenFormat } = await import('@/lib/auth/email-verification');
