...
describe("NextAuth Session Management with Cloud SQL", () => {
  ...
  describe("Session Persistence", () => {
    it("should persist session across requests", async () => {
      ...
      expect(secondRequestSession.user.email).toBe(firstRequestSession.user.email);
    });
  ...
  describe("Database Integration", () => {
    ...
    it("should handle concurrent database queries", async () => {
      ...
      const [result1, result2] = await Promise.all([
        db.select().from(users).where(eq(users.email, mockUsers[0].email)).limit(1),
        db.select().from(users).where(eq(users.email, mockUsers[1].email)).limit(1),
      ]);
