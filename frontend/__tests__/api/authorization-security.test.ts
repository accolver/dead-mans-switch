...
import { GET as getSecret, DELETE as deleteSecret } from '@/app/api/secrets/[id]/route';
...
describe('Authorization Security - API Integration Tests', () => {
  ...
  describe('Secrets API - GET /api/secrets/[id]', () => {
    ...
    it('should allow user to access their own secret', async () => {
      ...
      expect(mockSecretsService.getById).toHaveBeenCalledWith('secret-123', 'user-123');
    });
    ...
    it('should prevent cross-user secret access', async () => {
      ...
      expect(mockSecretsService.getById).toHaveBeenCalledWith('secret-123', 'user-456');
    });
  });
