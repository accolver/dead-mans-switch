...
describe('API Endpoints - Subscription Downgrade (TDD)', () => {
  ...
  describe('POST /api/user/subscription/schedule-downgrade', () => {
    it('should schedule downgrade for authenticated Pro user', async () => {
      ...
      const { POST } = await import('@/app/api/user/subscription/schedule-downgrade/route');
      
      const mockRequest = new NextRequest('http://localhost:3000/api/user/subscription/schedule-downgrade', {
        method: 'POST',
      });
      ...
      expect(subscriptionService.scheduleDowngrade).toHaveBeenCalledWith('user-123');
    });
  ...
  describe('POST /api/user/subscription/cancel-downgrade', () => {
    it('should cancel scheduled downgrade for authenticated user', async () => {
      ...
      const { POST } = await import('@/app/api/user/subscription/cancel-downgrade/route');
      
      const mockRequest = new NextRequest('http://localhost:3000/api/user/subscription/cancel-downgrade', {
        method: 'POST',
      });
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(subscriptionService.cancelScheduledDowngrade).toHaveBeenCalledWith('user-123');
    });
