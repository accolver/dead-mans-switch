...
describe('Cron Job - Process Subscription Downgrades (TDD)', () => {
  ...
  describe('POST /api/cron/process-subscription-downgrades', () => {
    it('should return 401 for missing authorization header', async () => {
      const { POST } = await import('@/app/api/cron/process-subscription-downgrades/route');
      
      const mockRequest = new NextRequest('http://localhost:3000/api/cron/process-subscription-downgrades', {
        method: 'POST',
      });
    ...
    it('should process eligible downgrades with valid CRON_SECRET', async () => {
      ...
      expect(subscriptionService.executeScheduledDowngrade).toHaveBeenCalledTimes(2);
    });
