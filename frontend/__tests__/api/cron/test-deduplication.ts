...
async function main() {
  ...
  if (!check2) {
    ...
    allReminders.forEach((r, i) => {
      console.log(
        `  ${i + 1}. ID: ${r.id}, Type: ${r.reminderType}, Status: ${r.status}, SentAt: ${r.sentAt}`,
      )
    })
  ...
  console.log(
    "Check 3: Checking if different reminder type exists (should be false)...",
  )
