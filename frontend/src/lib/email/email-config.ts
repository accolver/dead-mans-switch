...
export function validateCronSecret(): ValidationResult {
  ...
  if (cronSecret.length < MIN_CRON_SECRET_LENGTH) {
    return {
      valid: true,
      warning: `CRON_SECRET is too short (minimum ${MIN_CRON_SECRET_LENGTH} characters recommended for security)`,
    }
  }
...
export function validateEmailEnvironment(): EmailEnvironmentValidation {
  ...
  if (envProvider) {
    ...
    warnings.push(`EMAIL_PROVIDER not set, defaulting to ${provider}`)
  }
...
export function getEmailConfigurationStatus(): EmailConfigStatus {
  ...
  const allVars = [...requiredVars, ...getOptionalEmailVars(provider)]
...
export function validateEmailConfigAtStartup(): void {
  ...
  if (validation.warnings.length > 0) {
    console.warn("[EmailConfig] Configuration warnings:", validation.warnings)
  }
