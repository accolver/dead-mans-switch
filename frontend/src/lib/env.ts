...
function getEnvVar(name: string, required: boolean = true): string {
  ...
  if (!isServer && !isBuildTime) {
    console.warn(
      `Attempted to access ${name} on client side. Use ConfigContext instead.`,
    )
    return ""
  }
...
const NEXT_PUBLIC_PARENT_COMPANY = getEnvVar(
  "NEXT_PUBLIC_PARENT_COMPANY",
  false,
)
const NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY = getEnvVar(
