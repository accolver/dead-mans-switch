...
class SubscriptionService {
  ...
  private async handleCheckoutSessionCompleted(event: any, userId: string) {
    ...
    if (session.mode === "subscription" && session.subscription) {
      console.log(
        `[Subscription] Checkout completed for user ${userId}, subscription: ${session.subscription}`,
      )
  ...
  private async createOrUpdateSubscriptionFromCheckout(
    ...
    try {
      ...
      if (existingSubscription) {
        console.log(
          `[Subscription] Updating existing subscription for user ${userId}`,
        )
        ...
        console.log(
          `[Subscription] Creating new subscription for user ${userId}`,
        )
