...
class ConnectionManager {
  ...
  private openCircuitBreaker(): void {
    ...
    this.circuitBreakerResetTime = new Date(Date.now() + this.CIRCUIT_BREAKER_RESET_MS);
    console.error(`🚨 Circuit breaker OPEN - will retry at ${this.circuitBreakerResetTime.toISOString()}`);
  }

  async getConnection(connectionString: string, options: ConnectionOptions = {}): Promise<ReturnType<typeof postgres>> {
    // Check circuit breaker status
    this.checkCircuitBreaker();

    if (this.circuitBreakerOpen) {
      throw new Error(`Database circuit breaker is OPEN. Will reset at ${this.circuitBreakerResetTime?.toISOString()}`);
    }
    ...
    for (let attempt = 1; attempt <= this.retryConfig.maxAttempts; attempt++) {
      try {
        ...
        console.log(`✅ Database connected successfully to: ${testResult[0].db}`);
        ...
        if (this.connectionAttempts >= this.CIRCUIT_BREAKER_THRESHOLD) {
          ...
          throw new Error(`Database connection failed after ${attempt} attempts. Circuit breaker opened.`);
        }
        ...
        if (attempt < this.retryConfig.maxAttempts) {
          ...
          throw new Error(`Database connection failed after ${this.retryConfig.maxAttempts} attempts: ${error.message}`);
        }
      }
    }
