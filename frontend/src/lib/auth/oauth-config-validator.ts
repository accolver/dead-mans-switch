...
export function validateOAuthConfig(config?: OAuthConfig): ValidationResult {
  ...
  const nextAuthSecret = config ? config.nextAuthSecret : process.env.NEXTAUTH_SECRET
  ...
  if (!googleClientId) {
    ...
    errors.push('GOOGLE_CLIENT_ID must be a valid Google OAuth client ID ending with .apps.googleusercontent.com')
  }
  ...
  if (process.env.NODE_ENV === 'development') {
    if (nextAuthUrl?.startsWith('http://')) {
      warnings.push('Using HTTP in development is acceptable, but ensure HTTPS is used in production')
    }
  }
