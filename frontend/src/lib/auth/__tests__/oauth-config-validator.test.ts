...
describe('OAuth Config Validator', () => {
  ...
  describe('validateOAuthConfig', () => {
    ...
    it('should detect missing required fields when explicitly undefined', () => {
      ...
      expect(result.errors).toContain('GOOGLE_CLIENT_ID is required for Google OAuth')
      expect(result.errors).toContain('GOOGLE_CLIENT_SECRET is required for Google OAuth')
      expect(result.errors).toContain('NEXTAUTH_SECRET is required for NextAuth')
    ...
    it('should validate Google Client ID format', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('GOOGLE_CLIENT_ID must be a valid Google OAuth client ID ending with .apps.googleusercontent.com')
    })
    ...
    it('should validate NextAuth URL format', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('NEXTAUTH_URL must be a valid URL starting with http:// or https://')
    })
    ...
    it('should warn about default values', () => {
      ...
      expect(result.isValid).toBe(true)
      expect(result.warnings).toContain('NEXTAUTH_SECRET should be changed from the default value')
    })
    ...
    it('should warn about short secrets', () => {
      ...
      expect(result.isValid).toBe(true)
      expect(result.warnings).toContain('GOOGLE_CLIENT_SECRET seems too short, verify it is correct')
      expect(result.warnings).toContain('NEXTAUTH_SECRET should be at least 32 characters long for security')
    })
  })
  ...
  describe('assertValidOAuthConfig', () => {
    ...
    it('should throw for invalid configuration', () => {
      ...
      expect(() => assertValidOAuthConfig(config)).toThrow('OAuth configuration invalid:')
    })
  })
