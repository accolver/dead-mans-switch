...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check rate limit
    const rateLimitResult = await checkRateLimit('resend-verification', normalizedEmail);
    if (!rateLimitResult.allowed) {
      ...
      response.headers.set('Retry-After', rateLimitResult.retryAfter?.toString() || '1800');
    ...
    console.log(`[ResendVerification] Verification email sent for: ${normalizedEmail}`);
