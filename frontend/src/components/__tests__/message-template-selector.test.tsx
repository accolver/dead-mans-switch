...
describe("MessageTemplateSelector", () => {
  ...
  describe("Pro tier", () => {
    ...
    it("should display all 7 templates when dialog is open", async () => {
      ...
      expect(screen.queryAllByText("Bitcoin Wallet Access").length).toBeGreaterThan(0)
      expect(
        screen.queryAllByText("Password Manager Master Password").length
      ).toBeGreaterThan(0)
      expect(screen.queryAllByText(/Estate Planning/i).length).toBeGreaterThan(0)
      expect(
        screen.queryAllByText(/Safe Deposit Box/i).length
      ).toBeGreaterThan(0)
      expect(
    ...
    it("should show all templates when dialog opens", async () => {
      ...
      expect(screen.getByText(/choose a pre-written template/i)).toBeInTheDocument()
    })
  })
})
